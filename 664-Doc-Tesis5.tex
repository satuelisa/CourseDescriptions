...
\subsection{Clave y nombre de la Unidad de Aprendizaje:} \fbox{664 Tesis 5}
...
\subsection{Requisito:} \underline{Tesis 4}
...
para la tesis};
\\
\node[elem] (lit) {Actualizar literatura cient\'{\i}fica relacionada relevante};
&
\node[elem] (des) {Desarrollar la soluci\'{o}n propuesta};
&
\node[elem] (her) {Revisar herramientas computacionales
  existentes para implementar el m\'{e}todo};
\\
\node[elem] (for) {Actualizar las hip\'{o}tesis para la validaci\'{o}n del
  modelo propuesto};
& &
...
\\
&
\node[elem] (ana) {Dise\~{n}ar experimentos para
...
\item Un primer borrador de la redacci\'{o}n de la metodolog\'{\i}a
...
reportando lo discutido y avanzado en cada semana. Siendo quinto
...
unidad de aprendizaje (Tesis 5, en este caso), semestre en que se
