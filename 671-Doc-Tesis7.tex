...
\subsection{Clave y nombre de la Unidad de Aprendizaje:} \fbox{671 Tesis 7}
...
Se apoya al participante en la finalizaci\'{o}n de su trabajo de {\em
  tesis de doctorado}.
...
&
\node[elem] (des) {Actualiar la soluci\'{o}n propuesta};
\\
\node[elem] (for) {Actualizar las hip\'{o}tesis para la validaci\'{o}n del
  modelo propuesto};
\\
\node[elem] (dis) {Actualizar el dise\~{n}o del m\'{e}todo};
&
\node[elem] (imp) {Actualizar la implementaci\'{o}n del m\'{e}todo};
\\
&
\node[elem] (ana) {Realizar experimentos a profundidad};
\\ 
& & \node[elem] (doc) {Documentar los resultados experimentales conforme al estilo de
...
portafolio, otorgando por m\'{a}ximo 5 puntos por semana
...
reportando lo discutido y avanzado en cada semana. Siendo s\'{e}ptimo
semestre, se espera que un alumno concluya ya algunos 
cap\'{\i}tulos --- se visitan de forma sistem\'{a}tica
